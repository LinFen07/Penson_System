<template>
  ...
  >
    ...
    <div 
      ...
      @mousedown="mode === 'dialog' && startDrag"
    >
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <div class="relative">
        <NoteList 
          ...
          :style="{ width: `${sidebarWidth}px`, minWidth: '240px' }"
        />
        <!-- 列表宽度调整手柄 - 仅在弹窗模式显示 -->
        <div 
          v-if="mode === 'dialog'"
...
const handleDrag = (e: MouseEvent) => {
  ...
  position.value = {
    x: Math.max(0, Math.min(e.clientX - dragState.value.startPos.x, maxX)),
    y: Math.max(0, Math.min(e.clientY - dragState.value.startPos.y, maxY))
  }
}
...
const startResizeDialog = (e: MouseEvent) => {
  ...
  dialogRef.value?.style.setProperty('transition', 'none')
  document.addEventListener('mousemove', handleResizeDialog)
...
const handleResizeDialog = (e: MouseEvent) => {
  ...
  const width = Math.max(700, Math.min(
    0.95 * window.innerWidth,
    dragState.value.startSize.width + (e.clientX - dragState.value.startPos.x)
  ))
  
  const height = Math.max(400, Math.min(
    0.95 * window.innerHeight,
    dragState.value.startSize.height + (e.clientY - dragState.value.startPos.y)
  ))
...
const stopResizeDialog = () => {
  ...
  dialogRef.value?.style.removeProperty('transition')
  document.removeEventListener('mousemove', handleResizeDialog)
...
const handleResizeSidebar = (e: MouseEvent) => {
  ...
  // 限制侧边栏宽度范围：240px ~ 窗口一半
  const maxWidth = Math.floor(window.innerWidth / 2)
  sidebarWidth.value = Math.max(
    240,
    Math.min(
      maxWidth,
      dragState.value.startSidebarWidth + (e.clientX - dragState.value.startPos.x)
    )
  )
}
